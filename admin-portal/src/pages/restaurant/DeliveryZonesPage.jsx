    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <div style={{ position: 'fixed', bottom: '24px', right: '24px', zIndex: 999, display: 'flex', flexDirection: 'column', gap: '8px' }}>
        {toasts.map((t) => (
          <div key={t.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px', padding: '10px 14px', borderRadius: '9px', backgroundColor: 'var(--bg-input)', border: '1px solid #222', fontSize: '12px', color: t.type === 'success' ? '#22c55e' : '#ef4444', minWidth: '200px' }}>
            {t.msg}
            <button onClick={() => setToasts((p) => p.filter((x) => x.id !== t.id))} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer' }}><FontAwesomeIcon icon={faTimes} /></button>
          </div>
        ))}
      </div>

      <div>
        <p style={{ margin: '0 0 4px', fontSize: '11px', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '1px' }}>RESTAURANT</p>
      ...
      {zoneGroups.length > 0 && (
        <div style={{ backgroundColor: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: '12px', overflow: 'hidden' }}>
          <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text-primary)' }}>Zone Groups</span>
          </div>
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <button
                    ...
                    style={{ background: 'none', border: 'none', color: group.active ? '#22c55e' : 'var(--text-muted)', cursor: 'pointer', fontSize: '14px' }}
                  >
              ...
              {expandedGroup === group.id && (
                <div style={{ padding: '8px 16px 12px 40px' }}>
                  {group.zones?.map((zone) => (
                    <div key={zone.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid var(--border)' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
      ...
      {showGroupForm && (
        <div style={{ backgroundColor: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: '12px', padding: '20px' }}>
          <p style={{ margin: '0 0 14px', fontSize: '13px', fontWeight: 600, color: 'var(--text-primary)' }}>
          ...
          <form onSubmit={addGroup} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
              <div>
                <label style={{ display: 'block', fontSize: '11px', color: 'var(--text-muted)', marginBottom: '5px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Group Name</label>
                <input
                  type="text"
                  placeholder="e.g. Lagos Mainland"
                  value={groupForm.name}
                  onChange={(e) => setGroupForm((p) => ({ ...p, name: e.target.value }))}
                  style={{ width: '100%', backgroundColor: 'var(--bg-input)', border: '1px solid var(--border)', borderRadius: '8px', color: 'var(--text-primary)', padding: '9px 12px', fontSize: '13px', outline: 'none', boxSizing: 'border-box' }}
                />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: '11px', color: 'var(--text-muted)', marginBottom: '5px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Description</label>
                <input
                  type="text"
            ...
            <div>
              <label style={{ display: 'block', fontSize: '11px', color: 'var(--text-muted)', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
      ...
      <div style={{ backgroundColor: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: '12px', padding: '20px' }}>
        ...
        <form onSubmit={addZone} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 120px auto', gap: '10px', alignItems: 'end' }}>
          {[
            ...
            <div key={id}>
              <label style={{ display: 'block', fontSize: '11px', color: 'var(--text-muted)', marginBottom: '5px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{label}</label>
      ...
      <div style={{ backgroundColor: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: '12px', overflow: 'hidden' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 100px 80px 120px', padding: '10px 16px', borderBottom: '1px solid var(--border)' }}>
          {['Zone', 'Keywords', 'Fee', 'Status', 'Actions'].map((h) => (
            <p key={h} style={{ margin: 0, fontSize: '10px', fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.6px' }}>{h}</p>
          ))}
        </div>
        {zones.length === 0 ? (
          <p style={{ textAlign: 'center', padding: '40px', color: 'var(--text-muted)', fontSize: '13px' }}>No delivery zones yet</p>
        ) : zones.map((z, idx) => (
          <div key={z.id} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 100px 80px 120px', padding: '13px 16px', borderBottom: idx < zones.length - 1 ? '1px solid var(--border)' : 'none', alignItems: 'center', transition: 'background 0.15s' }}
            onMouseEnter={(e) => e.currentTarget.style.backgroundColor = 'var(--bg-input)'}
            onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'}
          >
            {editing?.id === z.id ? (
              <>
                <input value={editing.name} onChange={(e) => setEditing((p) => ({ ...p, name: e.target.value }))} style={{ backgroundColor: 'var(--bg-elevated)', border: '1px solid var(--border-focus)', borderRadius: '6px', color: 'var(--text-primary)', padding: '6px 9px', fontSize: '12px', outline: 'none' }} />
                <p style={{ margin: 0, fontSize: '12px', color: 'var(--text-secondary)' }}>{z.keywords}</p>
                <input type="number" value={editing.fee} onChange={(e) => setEditing((p) => ({ ...p, fee: e.target.value }))} style={{ width: '80px', backgroundColor: 'var(--bg-elevated)', border: '1px solid var(--border-focus)', borderRadius: '6px', color: 'var(--text-primary)', padding: '6px 9px', fontSize: '12px', outline: 'none' }} />
                ...
                <div style={{ display: 'flex', gap: '6px' }}>
                  ...
                  <button onClick={() => setEditing(null)} style={{ width: '28px', height: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'transparent', border: '1px solid var(--border)', borderRadius: '6px', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '11px' }}>
              ...
              <>
                <p style={{ margin: 0, fontSize: '13px', fontWeight: 600, color: 'var(--text-primary)' }}>{z.name}</p>
                <p style={{ margin: 0, fontSize: '12px', color: 'var(--text-secondary)' }}>{z.keywords}</p>
                <p style={{ margin: 0, fontSize: '13px', fontWeight: 600, color: '#22c55e' }}>₦{z.fee}</p>
                <div><Badge type={z.active ? 'active' : 'inactive'} label={z.active ? 'Active' : 'Off'} /></div>
                <div style={{ display: 'flex', gap: '6px' }}>
                  <button onClick={() => setEditing({ ...z })} style={{ width: '28px', height: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'transparent', border: '1px solid var(--border)', borderRadius: '6px', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '11px' }}><FontAwesomeIcon icon={faEdit} /></button>
                  <button onClick={() => toggleZone(z.id)} style={{ width: '28px', height: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'transparent', border: '1px solid var(--border)', borderRadius: '6px', color: z.active ? '#22c55e' : 'var(--text-muted)', cursor: 'pointer', fontSize: '11px' }}>
      ...
      {deleting && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.7)', zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ backgroundColor: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: '14px', padding: '24px', width: '340px', maxWidth: '90vw' }}>
            <p style={{ margin: '0 0 8px', fontSize: '14px', fontWeight: 700, color: 'var(--text-primary)' }}>Delete zone?</p>
            <p style={{ margin: '0 0 20px', fontSize: '13px', color: 'var(--text-secondary)' }}>"{deleting.name}" will be removed. This cannot be undone.</p>
            <div style={{ display: 'flex', gap: '8px' }}>
              <button onClick={() => setDeleting(null)} style={{ flex: 1, padding: '10px', background: 'transparent', border: '1px solid var(--border)', borderRadius: '8px', color: 'var(--text-secondary)', fontSize: '13px', cursor: 'pointer' }}>Cancel</button>
              <button onClick={deleteZone} style={{ flex: 1, padding: '10px', backgroundColor: '#ef4444', border: 'none', borderRadius: '8px', color: 'var(--text-primary)', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}>Delete</button>
            </div>
          </div>
        </div>
      )}
      ...
      {deletingGroup && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.7)', zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ backgroundColor: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: '14px', padding: '24px', width: '340px', maxWidth: '90vw' }}>
            <p style={{ margin: '0 0 8px', fontSize: '14px', fontWeight: 700, color: 'var(--text-primary)' }}>Delete group?</p>
            <p style={{ margin: '0 0 20px', fontSize: '13px', color: 'var(--text-secondary)' }}>"{deletingGroup.name}" will be removed. Zones will not be deleted.</p>
            <div style={{ display: 'flex', gap: '8px' }}>
              <button onClick={() => setDeletingGroup(null)} style={{ flex: 1, padding: '10px', background: 'transparent', border: '1px solid var(--border)', borderRadius: '8px', color: 'var(--text-secondary)', fontSize: '13px', cursor: 'pointer' }}>Cancel</button>
              <button onClick={deleteGroup} style={{ flex: 1, padding: '10px', backgroundColor: '#ef4444', border: 'none', borderRadius: '8px', color: 'var(--text-primary)', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}>Delete</button>
            </div>
          </div>
        </div>
      )}
