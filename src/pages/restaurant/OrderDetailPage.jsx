...
const OrderDetailPage = () => {
  ...
  useEffect(() => {
    let active = true;
    ordersApi.getById(user.restaurantId, orderId).then((response) => {
      if (active) {
        setOrder(response);
        if (response?.status) {
          setNextStatus(response.status);
      ...
      <div className="card">
        ...
        <div className="actions-row wrap">
          {actions.map((action) => (
            <button
              ...
              onClick={() => runAction(action.handler)}
            >
              {action.label}
            </button>
          ))}
        ...
        <div className="inline-form-row">
          ...
          <button
            ...
            className="button"
            onClick={async () => {
              await ordersApi.updateStatus(user.restaurantId, order.id, nextStatus);
              load();
            }}
          >
            Update Status
